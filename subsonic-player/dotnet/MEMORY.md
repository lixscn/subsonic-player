...
## 2026-09-16 会话（三续）：把文件真正搬进「对的目录」+ 两个自伤 bug

用户问「对完 AcoustID 后改了名的有没有移到对的目录里？」——没有，而且这一问查出两个我自己造的 bug。

### bug 1：路径前缀混用（1447 个文件被搬进嵌套镜像）
`acoustid-diff.py` 在**宿主机**跑，所以计划里的 `new.path` 是 `/vol1/@team/public/music/...`；
`acoustid-apply.py` 在**容器**里跑，于是 `not target.startswith("/app/media")` 成立，
`os.path.join(ROOT, target.lstrip("/"))` 得到 `/app/media/vol1/@team/public/music/...` ——
**把宿主机路径整条塞进了媒体根目录**，1447 个文件被搬进 `/music/vol1/@team/public/music/…`。

**为什么验收没发现**：我写的是「磁盘文件集合 ↔ 数据库 path 集合」双向比对，
两边**一致地错**所以全绿。⇒ **验收要校验语义结构（顶层目录是否等于艺术家），不能只校验两边相等。**
这也是「同名/同集合」这类判据本次第 N 次坑我。

### bug 2：直接 rename 造成互换覆盖（2 个文件永久丢失）
第一版 `rehome-all.py` 直接 `os.rename(from, to)`。当**两行互换路径**（A 的目标恰好是 B 的当前路径）时：
A 先执行 → 覆盖 B 的文件；B 再执行 → 把 A 的内容挪到 B 的目标。结果 B 的音频没了、A 的行悬空。
- 受害 1：`刘若英/我等你/`（`后来` 341.4s ↔ `原来你也在这里` 219.5s）
- 受害 2：`阿杜/天黑/`（`Andy` 315.09s ↔ `他一定很爱你` 210.83s）
- ⚠️ DB 写入还因为 `path_hash` 唯一索引抛了 `IntegrityError` —— **文件已经搬了、DB 没更新**，
  于是「行悬空」被我当成普通的保存失败，差点漏掉。

**正确做法**：要么**两步走**（`from → from.tmp`，再把所有 `*.tmp → 最终名`），
要么先把全部源路径登记为「待释放」再移动。**永远不要直接 rename 到可能被别的行占用的目标名。**

**定位修复**：用 **ffprobe 时长**比对存留文件属于哪一行（341.4s 属 `后来`、315.09s 属 `Andy`），
把保留的音频归位到正确名字，删掉音频已被覆盖的那 2 行。曲目 4301 → **4299**。

### 结果
`rehome-all.py`：2501 次移动（含 1447 个逃出嵌套镜像），删 1209 个空目录（含整个 `vol1/` 镜像）。
终态 **0 个文件在错的歌手目录下**，4299 ↔ 4299 一一对应，806 个顶层目录，75GB。

## 2026-09-16 m4a 流式修复（faststart remux）

**问题**：客户端对 m4a 报 `BASS_ERROR_UNSTREAMABLE`，只能退回「整文件下载再播」。
**根因不是编码格式，也不是服务器**：实测服务器对 m4a 发 Range 请求返回正常（206 + `audio/x-m4a`），
和 flac/wav 一样。真正原因是 **MP4 容器的 moov（索引）在文件尾部**，BASS 必须先扫到文件末尾才能解码。

**正确做法是 remux，不是 transcode**：`ffmpeg -c copy -movflags +faststart`
把索引挪到文件头部 —— **音频流原样复制、不重编码、不损音质、格式不变**，约 0.6~0.8 秒/文件。
转成 mp3/flac 要么损音质要么体积暴涨，完全没必要。

**执行**：全库 2804 个 mp4/m4a 里原本 **1493 个非 faststart**，全部 remux 完成，
逐个比对音频流 MD5（`-t 60 -map 0:a -f md5`）—— **0 个失败、0 个 MD5 不符**。
现在 **2804/2804 全部 faststart**；磁盘 ↔ 数据库 4299 ↔ 4299。

**格式盘点**（客户端自带 `bassdsd`/`bassape` 插件，所以这两种不用转）：
m4a 2804 / flac 791 / wav 559 / dsf 69 / mp3 56 / ape 20。

**排查思路记一笔**：判断「能不能流式」要看**容器索引位置**，不要一上来就想转码；
`atom_order()` 顺序读顶层 box（ftyp/free/mdat/moov）就能判定，不必依赖任何工具。
