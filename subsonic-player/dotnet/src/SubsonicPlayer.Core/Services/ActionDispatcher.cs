...
public sealed class UiTimer
{
    ...
    /// <summary>停止（已释放时为无害的空操作）。</summary>
    public void Stop()
    {
    ...
    /// <summary>
    /// 停止并释放底层计时器。进程退出路径调用：不释放会让计时器回调在线程池里继续排队，
    /// 退出时可能再次触发业务回调（例如播放状态写库），拖住关机。
    /// 释放后 <see cref="Stop"/> / <see cref="Dispose"/> 可重复调用（内部吞掉 ObjectDisposedException）。
    /// </summary>
    public void Dispose()
    {
