...
public class FavoritesService
{
    ...
    private async Task LoadInternalAsync()
    {
        ...
        // 1) 标准 OpenSubsonic getStarred：Music Tap/Music Tag 等按「歌曲级」星标返回收藏。
        //    （Gonic 的 getStarred 有 bug，可能返回脏 id，但脏 id 不会匹配到真实歌曲，故无害。）
        try
        {
            var starred = await music.GetStarredSongsAsync();
            foreach (var s in starred)
        ...
        // 2) Gonic 的 getStarred 不可靠，用「喜欢的音乐」歌单兜底；若 Music Tap 也有同名歌单则一并并入。
        try
        {
