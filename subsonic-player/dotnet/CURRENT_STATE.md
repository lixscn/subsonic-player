...
## 2026-09-16 补齐：把文件真正搬进「对的目录」

用户问「对完 AcoustID 后，改了名的有没有移到对的目录里？」—— **答案是没有**，而且查出一个更严重的问题。

1. **`acoustid-apply.py` 只做了「原地改名」**（`dest = 同目录 + 新文件名`），没搬目录。
2. **更糟：路径前缀 bug。** 计划里的 `new.path` 是**宿主机**路径 `/vol1/@team/public/music/...`，
   apply 里判断 `not target.startswith("/app/media")` 就把整条宿主机路径 `os.path.join(ROOT, ...)`，
   于是 **1447 个文件被搬进了 `/vol1/@team/public/music/vol1/@team/public/music/...` 这个嵌套镜像**。
   ⚠️ 我事后的「磁盘 ↔ 数据库」验收**看不出来**：两边是一致的，只是**一致地错**。
   ⇒ **验收必须校验结构**（顶层目录 == 艺术家），不能只比对两边是否相等。

3. **修复**：`rehome-all.py` 把所有文件搬到 `<media>/<歌手>/<专辑>/<歌手> - <歌名>.<ext>`：
   2501 次移动（含 1447 个逃出嵌套镜像），1209 个空目录（含整个 `vol1/` 镜像）删除。
   现在 **0 个文件还在错的歌手目录下**（4281 条歌手+专辑目录都对；18 条的专辑目录是 `未知`
   而数据库写 `未分类专辑`，属命名差异，不影响使用）。

4. **代价：2 个文件被覆盖（不可恢复）。** 我第一版移动逻辑直接 `os.rename(from, to)`，
   遇到**两行互换路径**（A 的目标正是 B 的当前路径）时，A 先跑就把 B 的文件覆盖了，
   B 再把自己的 rename 挪走，于是 B 的音频没了、A 的行悬空。
   受害：`刘若英/我等你/`（`后来` 341.4s 与 `原来你也在这里` 219.5s）、
   `阿杜/天黑/`（`Andy` 315.09s 与 `他一定很爱你` 210.83s）。
   用 **ffprobe 时长比对**确认留下来的音频属于哪一行 → 归位保留方、删掉音频已丢的 2 行。
   曲目 4301 → **4299**。
   ⇒ **批量改名/移动必须两步走**（先改成临时名再改成目标名），或先把所有源路径登记为「待释放」，
   **绝不能直接 rename 到可能被别的行占用的目标**。

**终态**：4299 曲目 / 2069 专辑 / 2162 艺术家；806 个顶层目录；75GB；
磁盘 ↔ 数据库 **4299 ↔ 4299**，无孤立/无缺失/无重复路径；`search3` 分页合计 4299。
