...
同一套服务端、同一套曲库，另有一个**原生 Android 客户端**：`android/`（纯 Java + Android Framework，零第三方依赖，可用本机 SDK 命令行工具链完全离线构建成 APK）。

- 界面按手机竖屏重做：底部 5 Tab（发现/曲库/收藏/搜索/设置）+ 迷你播放条 + 全屏播放页（卡拉OK 歌词 / 队列编辑）
- 播放走系统 `MediaPlayer` + 前台服务 + `MediaSession`：后台播放、通知栏与锁屏控制、耳机按键、音频焦点
- 6 套主题 + 强调色、收藏/评分/歌单管理、LRCLIB + 网易云歌词兜底

详见 [`android/README.md`](android/README.md)。构建：`cd android && powershell -File build.ps1 -Install`。

> 与桌面版的差异：移动端暂不含 BASS 音效链路（EQ/DSP/频谱）、Gapless 与淡入淡出，且因服务端不转码，DSD/APE/WavPack 等格式不可播（mp3 / m4a / flac / wav / ogg 正常）。
